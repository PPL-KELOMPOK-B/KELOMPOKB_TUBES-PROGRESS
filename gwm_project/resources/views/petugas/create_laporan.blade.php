        ...
        <div class="profile-container">
            <div class="profile-top">
                ...
                <div class="profile-info">
                    <h4>Petugas</h4>
                    <p>{{ auth()->user()->nama ?? 'Petugas Kecamatan' }}</p>
                </div>
            </div>
        ...
        <form action="{{ isset($laporan) ? route('petugas.update_laporan', $laporan->id) : route('petugas.store_laporan') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card" style="margin-bottom: 24px;">
                ...
                <div style="display: grid; gap: 20px;">
                    <div>
                        ...
                        <input type="text" readonly value="{{ auth()->user()->kecamatan->nama_kecamatan ?? 'Purwosari' }}" style="width: 100%; padding: 12px 16px; border-radius: 8px; border: 1px solid var(--border); background-color: #f8fafc; color: var(--text-gray); font-family: inherit; font-size: 14px; outline: none;">
                    ...
                    <div>
                        ...
                        <select name="kelurahan_id" required style="width: 100%; padding: 12px 16px; border-radius: 8px; border: 1px solid {{ $errors->has('kelurahan_id') ? '#ef4444' : '#cbd5e1' }}; background-color: white; color: var(--text-dark); font-family: inherit; font-size: 14px; outline: none; appearance: none; cursor: pointer; background-image: url('data:image/svg+xml;utf8,<svg fill=%22none%22 stroke=%22currentColor%22 viewBox=%220 0 24 24%22 xmlns=%22http://www.w3.org/2000/svg%22><path stroke-linecap=%22round%22 stroke-linejoin=%22round%22 stroke-width=%222%22 d=%22M19 9l-7 7-7-7%22></path></svg>'); background-repeat: no-repeat; background-position: right 16px center; background-size: 16px;">
                            <option value="">Pilih Kelurahan</option>
                            @foreach($kelurahans as $k)
                                <option value="{{ $k->id }}" {{ (isset($laporan) && $laporan->kelurahan_id == $k->id) || old('kelurahan_id') == $k->id ? 'selected' : '' }}>{{ $k->nama_kelurahan }}</option>
                            @endforeach
                        </select>
                        @error('kelurahan_id')
                ...
                <div style="display: grid; gap: 20px;">
                    <div>
                        ...
                        <select name="kondisi_air" required style="width: 100%; padding: 12px 16px; border-radius: 8px; border: 1px solid {{ $errors->has('kondisi_air') ? '#ef4444' : '#cbd5e1' }}; background-color: white; color: var(--text-dark); font-family: inherit; font-size: 14px; outline: none; appearance: none; cursor: pointer; background-image: url('data:image/svg+xml;utf8,<svg fill=%22none%22 stroke=%22currentColor%22 viewBox=%220 0 24 24%22 xmlns=%22http://www.w3.org/2000/svg%22><path stroke-linecap=%22round%22 stroke-linejoin=%22round%22 stroke-width=%222%22 d=%22M19 9l-7 7-7-7%22></path></svg>'); background-repeat: no-repeat; background-position: right 16px center; background-size: 16px;">
                            ...
                            <option value="aman" {{ (isset($laporan) && $laporan->kondisi_air == 'aman') || old('kondisi_air') == 'aman' ? 'selected' : '' }}>Aman</option>
                            <option value="waspada" {{ (isset($laporan) && $laporan->kondisi_air == 'waspada') || old('kondisi_air') == 'waspada' ? 'selected' : '' }}>Waspada</option>
                            <option value="siaga" {{ (isset($laporan) && $laporan->kondisi_air == 'siaga') || old('kondisi_air') == 'siaga' ? 'selected' : '' }}>Siaga</option>
                            <option value="kritis" {{ (isset($laporan) && $laporan->kondisi_air == 'kritis') || old('kondisi_air') == 'kritis' ? 'selected' : '' }}>Kritis</option>
                        </select>
                        @error('kondisi_air')
                            <span style="font-size: 12px; color: #ef4444; display: block; margin-top: 6px;">{{ $message }}</span>
                        @enderror
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div>
                            ...
                            <input type="number" name="jumlah_terdampak" placeholder="0" required value="{{ old('jumlah_terdampak', $laporan->jumlah_terdampak ?? '') }}" style="width: 100%; padding: 12px 16px; border-radius: 8px; border: 1px solid {{ $errors->has('jumlah_terdampak') ? '#ef4444' : '#cbd5e1' }}; background-color: white; color: var(--text-dark); font-family: inherit; font-size: 14px; outline: none;">
                            @error('jumlah_terdampak')
                                <span style="font-size: 12px; color: #ef4444; display: block; margin-top: 6px;">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="number" name="durasi_hari" placeholder="0" required value="{{ old('durasi_hari', $laporan->durasi_hari ?? '') }}" style="width: 100%; padding: 12px 16px; border-radius: 8px; border: 1px solid {{ $errors->has('durasi_hari') ? '#ef4444' : '#cbd5e1' }}; background-color: white; color: var(--text-dark); font-family: inherit; font-size: 14px; outline: none;">
                            @error('durasi_hari')
                                <span style="font-size: 12px; color: #ef4444; display: block; margin-top: 6px;">{{ $message }}</span>
                            @enderror
                        </div>
            ...
            <div class="card" style="margin-bottom: 24px;">
                ...
                <script>
                    ...
                    @if(isset($laporan) && $laporan->fotos->count() > 0)
                        @php
                            $fotos = $laporan->fotos->pluck('path_foto')->toArray();
                        @endphp
            ...
            <div class="card" style="margin-bottom: 32px;">
                ...
                <textarea name="catatan" required rows="4" placeholder="Jelaskan kondisi kekeringan secara detail..." style="width: 100%; padding: 16px; border-radius: 8px; border: 1px solid {{ $errors->has('catatan') ? '#ef4444' : '#cbd5e1' }}; background-color: white; color: var(--text-dark); font-family: inherit; font-size: 14px; outline: none; resize: vertical;">{{ old('catatan', $laporan->catatan ?? '') }}</textarea>
                @error('catatan')
                    <span style="font-size: 12px; color: #ef4444; display: block; margin-top: 6px;">{{ $message }}</span>
                @enderror
            </div>
