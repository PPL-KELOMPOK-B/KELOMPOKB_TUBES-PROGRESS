...
class DashboardController extends Controller
{
    ...
    public function adminMonitoring(Request $request)
    {
        ...
        // 2. Bar Chart: Monthly Dryness Composition (Jan - Dec)
        $kritisData = [];
        $tinggiData = [];
        $sedangData = [];
        ...
        for ($m = 1; $m <= 12; $m++) {
            if (isset($laporansByMonth[$m])) {
                $kritisData[] = $laporansByMonth[$m]->filter(fn($lap) => $lap->tingkat === 'Kritis')->count();
                $tinggiData[] = $laporansByMonth[$m]->filter(fn($lap) => $lap->tingkat === 'Tinggi')->count();
                $sedangData[] = $laporansByMonth[$m]->filter(fn($lap) => $lap->tingkat === 'Sedang')->count();
            } else {
                $kritisData[] = 0;
                $tinggiData[] = 0;
                $sedangData[] = 0;
    ...
    public function riwayatKondisi(Request $request)
    {
        ...
        // Calculate counts based on ALL matching search term (ignoring kondisi filter for the top cards)
        $totalLaporan = $laporans->count();
        $sedangCount = $laporans->where('tingkat', 'Sedang')->count();
        $tinggiCount = $laporans->where('tingkat', 'Tinggi')->count();
        $kritisCount = $laporans->where('tingkat', 'Kritis')->count();

        if ($request->filled('kondisi') && $request->kondisi !== 'Semua Kondisi') {
            $laporans = $laporans->where('tingkat', $request->kondisi)->values();
        }

        return view('admin.riwayat_kondisi', compact(
            'laporans',
            'totalLaporan',
            'sedangCount',
            'tinggiCount',
